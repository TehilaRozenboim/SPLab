...
int main(int argc , char **argv) {
        ...
        if (strcmp(argv[i], "-o") == 0) {
            ...
            if (argc > i + 1) {
                if (legal_int(argv[i + 1]) == 1) {
                    num_of_changes_index = i + 1;
                    i++;
                }
            }
        } else if (origin_file_index == 0) {
            origin_file_index = i;
        } else if (new_file_index == 0) {
            new_file_index = i;
        }
    }
    ...
    if (tFlag)
        ...
        if (num_of_changes_index != -1)
            sscanf (argv[num_of_changes_index], "%ld", &size_of_diff);
        restore(diff_list, newFile, size_of_diff);
    }
    else {
