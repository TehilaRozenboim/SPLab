...
/* My additions */
cmdLine *copy_cmd_line(cmdLine *pCmdLine) {
    cmdLine *new_cmd_line = (cmdLine *)malloc(sizeof(cmdLine));
    int i;
    
    new_cmd_line->argCount = pCmdLine->argCount;
    new_cmd_line->inputRedirect = NULL;
    new_cmd_line->outputRedirect = NULL;
    new_cmd_line->blocking = pCmdLine->blocking;
    new_cmd_line->idx = pCmdLine->idx;
    new_cmd_line->next = NULL;

    
    for (i = 0; i < pCmdLine->argCount; i++) {
        ((char **)new_cmd_line->arguments)[i] = strClone(pCmdLine->arguments[i]);
    }
